	...
	glm::vec4* dL_drot)
{
	...
	// Compute gradient updates due to computing depth from 3D mean
	if (dLd_ddepth)
...
renderCUDA(
	...
	const float* __restrict__ dynamics,   // arg

	const float* __restrict__ final_Ts,
	const uint32_t* __restrict__ n_contrib,
	const float* __restrict__ dL_dpixels,
	const float* __restrict__ dLdy_dpixels,  // know  
	...
	float* __restrict__ dLdy_ddynamics  // unknow
	)
{
	...
	__shared__ float collected_dynamics[BLOCK_SIZE];
	...
	if (inside){
		...
		if(dLdy_dpixels) dLdy_dpixel=dLdy_dpixels[pix_id];
	}
	float last_alpha = 0;
	float last_color[C] = { 0 };
	float last_depth = 0;
	float last_dynamic = 0;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			collected_dynamics[block.thread_rank()] = dynamics[coll_id];
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const float dpixel_ddynamic = dchannel_dcolor;
			...
			if (dLdy_dpixels){
				// We just consider dynamic value ,   do not train with other parameters
				const float dynamic = collected_dynamics[j]; // 像素接受到的动态值
				// Update last color (to be used in the next iteration)
				accum_rec_dynamic = last_alpha * last_dynamic + (1.f - last_alpha) * accum_rec_dynamic;
				last_dynamic = dynamic;
				atomicAdd(&(dLdy_ddynamics[global_id]), dLdy_dpixel * dpixel_ddynamic);
			}
