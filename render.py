...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, cam_type):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        rendering = render(view, gaussians, pipeline, background, cam_type=cam_type, stage= "fine")["render"]
        time2 = time()
...
def render_sets( opt , dataset : ModelParams, hyperparam, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, skip_video: bool):
    with torch.no_grad():
